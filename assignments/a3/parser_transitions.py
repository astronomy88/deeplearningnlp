...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        #-- Shift
        #   Takes the first element in buffer and puts it at the end of stack
        if transition == "S":
            self.stack.append(self.buffer.pop(0))

        #-- Left Arc
        #   Creates a dependency from the last item to the second-to-last item, and 
        #   removes the second-to-last item from stack
        if transition == "LA":
            target = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1], target))

        #-- Right Arc
        #   Creates dependency from second-to-last item to the last item, and
        #   Removes the last item from the stack
        if transition == "RA":
            target = self.stack.pop(-1)
            self.dependencies.append((self.stack[-1], target)) #-- The previously 2nd-to-last item is now the last item on stack
...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = partial_parses[:] #-- We do this so we can apply the 'del' operator, as explained in the above Note
